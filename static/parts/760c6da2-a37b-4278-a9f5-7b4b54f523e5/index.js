    ...
    Remove: {
        fun: function (sys, items, opts) {
            this.watch("remove", (e, p) => {
                window.app.dialog.confirm("确定该区域吗？", "温馨提示", () => {
                    this.trigger("publish", ["/areas/remove", {id: p.id}]);
                    this.watch("/areas/remove", (m, p) => {
    ...
    Content: {
        ...
        fun: function (sys, items, opts) {
            ...
            function callback(e, p) {
                ...
                e.target.trigger("publish", "/areas/select").trigger("switch", "overview");
            }
            function clear() {
                items.area.val("");
...
$_("signup/form").imports({
    ...
    Area: {
        xml: "<Input id='area' label='名称' placeholder='areaname' maxlength='32'/>",
        fun: function (sys, items, opts) {
            function error( msg ) {
                items.area.focus();
